...
<style>
  * { box-sizing: border-box; margin: 0; }
  ...
  .wrap { position: relative; display: grid; grid-template-columns: 1.05fr 1fr; gap: 40px; padding: 64px 72px 56px; height: 100%; }
  .kicker { display: inline-flex; align-items: center; gap: 10px; font-size: 20px; font-weight: 600; color: #bfdbfe; letter-spacing: .04em; }
  .kicker .dot { width: 10px; height: 10px; border-radius: 50%; background: #4ade80; box-shadow: 0 0 0 5px rgba(74,222,128,.25); }
  h1 { font-size: 54px; line-height: 1.15; white-space: nowrap; font-weight: 800; margin-top: 22px; letter-spacing: .01em; }
  h1 small { display: block; font-size: 26px; font-weight: 600; color: #c7d2fe; margin-top: 12px; letter-spacing: .02em; }
  .chips { display: flex; flex-wrap: wrap; gap: 10px; margin-top: 30px; }
  .chip { padding: 8px 16px; border-radius: 999px; background: rgba(255,255,255,.12); border: 1px solid rgba(255,255,255,.28); font-size: 20px; font-weight: 600; }
  .url { position: absolute; left: 72px; bottom: 44px; font-size: 22px; color: #bfdbfe; letter-spacing: .02em; }
  .card { align-self: center; background: #fff; color: #1f2430; border-radius: 18px; padding: 30px 32px; box-shadow: 0 30px 80px rgba(0,0,0,.35), 0 0 0 1px rgba(255,255,255,.25); transform: rotate(-2deg); }
  .card .title { font-size: 24px; font-weight: 800; text-align: center; margin-bottom: 18px; letter-spacing: .1em; }
  .row { font-size: 21px; line-height: 2; white-space: nowrap; }
  ...
  .stamp { position: absolute; right: 72px; bottom: 44px; display: flex; align-items: center; gap: 10px; font-size: 20px; color: #bfdbfe; }
  .stamp b { color: #fff; }
</style></head>
<body>
<div class="grid"></div>
<div class="wrap">
  <div>
    ...
    <h1>文件去識別化工具<small>個資自動偵測、遮罩預覽、編碼表還原</small></h1>
